...
rename(.data = alcohol, beer = beer_servings, spirit = spirit_servings, wine = wine_servings)

# Now an example for selecting columns, also not saving the changes:
select(.data = alcohol, -total_litres_of_pure_alcohol)
...
```{r}
pop_GDP <- pop_GDP %>% 
    mutate(GDP = GDP_bil * 10^9, 
           GDP_capita = GDP_capita_thou * 10^3, 
           Pop = Population_mil * 10^6)
```
...
```{r}
pop_GDP_alcohol <- inner_join(x = alcohol, y = pop_GDP)
```
...
```{r}
pop_GDP_alc_td <- pop_GDP_alcohol %>% 
    gather(key = dose_type, value = dose_number, wine, beer, spirit)
```
...
```{r}
pop_GDP_alc_td %>% 
    spread(key = dose_type, value = dose_number)
```
...
pop_GDP_alc_td <- pop_GDP_alc_td %>% 
    mutate(continent = if_else(condition = country == "USA", true = "america", false = if_else(condition = country %in% africa, true = "africa", false = if_else(condition = country %in% europe, true = "europe", false = "asia"))))
```
...
pop_GDP_alc_td %>% 
    group_by(continent) %>% 
    summarise(alc_dose_cont = sum(dose_number))
```

```{r}
pop_GDP_alc_td %>% 
    group_by(continent, dose_type) %>% 
    summarise(alc_dose_cont = sum(dose_number))
```
...
pop_GDP_alc_td %>% 
    group_by(continent) %>% 
    summarize(tt_alcohol = sum(dose_number), avg_GDP_pc = mean(GDP_pc))
```
...
```{r}
pop_GDP_alc_td %>% 
    filter(dose_type == "wine" & continent != "asia")
```
...
```{r}
pop_GDP_alc_td %>% 
    ggplot(aes(x = dose_number, y = GDP_pc)) +
    geom_point()
```
...
```{r}
pop_GDP_alc_td %>% 
    ggplot(aes(x = dose_number, y = GDP_pc, color = continent)) +
    geom_point()
```
...
pop_GDP_alc_td %>% 
    ggplot(aes(x = dose_number, y = GDP_pc, color = continent)) +
    geom_point() +
    facet_wrap(continent ~ dose_type)

#Sometimes facet_grid will look nicer in these case.
pop_GDP_alc_td %>% 
    ggplot(aes(x = dose_number, y = GDP_pc, color = continent)) +
    geom_point() +
    facet_grid(continent ~ dose_type)
```
...
```{r}
pop_GDP_alc_td %>% 
    ggplot(aes(x = dose_type, y = dose_number)) +
    geom_boxplot() +
...
```{r}
pop_GDP_alc_td %>% 
    ggplot(aes(x = GDP, y = GDP_pc, color = dose_number)) +
    geom_point() +
...
```{r}
pop_GDP_alc_td %>% 
    ggplot(aes(x = Pop, y = dose_number, color = dose_type)) +
    geom_point() +
    geom_smooth(method = "lm", se = F) + 
    labs(x = "Population", y = "Number of doses of alcohol consumed yearly", color = "Alcohol type", title = "Alcohol consumption by population for some countries", caption = "Data from world bank and FiveThirtyEight")
